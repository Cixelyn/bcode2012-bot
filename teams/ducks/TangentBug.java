...
public class TangentBug{
	 ...
	 * 0 for clockwise, 1 for counterclockwise, -1 for not tracing last turn
	 */
	...
	public TangentBug(Navigator nav) {
		this.nav = nav;
		reset();
	}
	public void reset() {
	...
	public int[] computeMove(boolean[][] map, int sx, int sy, int tx, int ty) {
		...
		for(int step=0; step<INITIAL_SCAN_RANGE; step++) {
			...
			int dirTowards = NavigatorUtilities.getDirTowards(tx-cx, ty-cy);
